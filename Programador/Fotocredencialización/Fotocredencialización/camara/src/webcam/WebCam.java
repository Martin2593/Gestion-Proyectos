 ...
 */
 public  class WebCam implements Runnable {
     ...
     * @see #getImage()
     */
    public volatile boolean cameraAvailable, starting;

    /**
     * Contiene el hilo que se va a encargar de iniciar la cámara.
     */
    public Thread initCam;
    ...
    public void run() {
        ...
        try {
            ...
            System.out.println("Error iniciando la WebCam: "+ex);
            System.exit(0);
        }
    } // fin del run();
     ...
     */
    public CaptureDeviceInfo getCamera() {
     ...
     */
    public VideoFormat getLargestVF(CaptureDeviceInfo device) {
     ...
     */
    public VideoFormat getSmallestVF(CaptureDeviceInfo device) {
        VideoFormat vFormat = null;
        Format[] formats = device.getFormats();
        for (int i = 0; i < formats.length; i++) {
            if (formats[i] instanceof VideoFormat) {
